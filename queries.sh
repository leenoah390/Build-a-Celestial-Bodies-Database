...
echo "$($PSQL "SELECT teams.name FROM games FULL JOIN teams ON teams.team_id=games.winner_id WHERE year=2014 AND round='Eighth-Final' UNION SELECT teams.name FROM games FULL JOIN teams ON teams.team_id=games.opponent_id WHERE year=2014 AND round='Eighth-Final' ORDER BY name")"
...
echo "$($PSQL "SELECT year, teams.name FROM games LEFT JOIN teams ON teams.team_id=games.winner_id WHERE round='Final' ORDER BY name DESC")"
