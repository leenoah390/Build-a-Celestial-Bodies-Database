...
do
  ...
  then
    ...
    WIN_NAME=$($PSQL "SELECT team_id FROM teams WHERE name='$WINNER'") # Recollect
    OPP_NAME=$($PSQL "SELECT team_id FROM teams WHERE name='$OPPONENT'")
    echo $($PSQL "INSERT INTO games(year, round, winner_id, opponent_id, winner_goals, opponent_goals) VALUES($YEAR, '$ROUND', $WIN_NAME, $OPP_NAME, $WINNER_GOALS, $OPPONENT_GOALS)")
  fi
done
